...
/**
 * Generated class for the ItemDetail_12Page page.
 *
...
@IonicPage()
@Component({
  selector: 'page-item-detail-12',
  templateUrl: 'item-detail-12.html',
})
export class ItemDetail_12Page {
  ...
  getAboutOnce() {
    ...
    that.aboutService.getAboutOnce(that.code).then((rs) => {
      if (rs.code == 0) {
        ...
        that.aboutList.current_code.forEach((v)=>{
          that.sum+=Number(v);
        });
        that.countDown();
        that.numberRoll();

...
/**
 * Generated class for the ParityTrendComponent component.
 *
...
declare var $:any;
@Component({
  selector: 'parity-trend',
  templateUrl: 'parity-trend.html'
})
export class ParityTrendComponent implements OnDestroy{
// text: string;
  @Input() code;
  @Input() date;
  ...
  constructor(private aboutService: AboutService, private alertService: AlertService, private loadingCtrl: LoadingController) {
    console.log('Hello ParityTrendComponent Component');
  }
  ...
  getAnalysisDetail() {
    ...
    that.change++;
    that.aboutService.getAnalysisDetail(that.code, 'dszs', that.date).then((res) => {
      if (res.code == 0) {

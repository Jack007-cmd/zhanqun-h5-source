 ...
 //    data["lotterySourceFrom"] = lotterySourceFrom;
    ...
    return uni.request({
        ...
        if (res[1].data.status && res[1].data.code == 200) {
            return getFontConversion(res[1])
        } else {
...
// 			return Promise.reject()
            return getFontConversion(res[1].data)
        }
    }).catch(parmas => {
// 		console.log(parmas);
// 　　　　　　switch (parmas.code) {
...
// 简体转为繁体
function getFontConversion(params) {
    function simpPYStr() {
    ...
    for (var i = 0; i < params.length; i++) {
        if (simpPYStr().indexOf(params.charAt(i)) != -1)
            str += ftPYStr().charAt(simpPYStr().indexOf(params.charAt(i)));
        else if (qqPYStr().indexOf(params.charAt(i)) != -1)
            str += ftPYStr().charAt(qqPYStr().indexOf(params.charAt(i)));
        else
            str += params.charAt(i);
    }
    str = JSON.parse(str);

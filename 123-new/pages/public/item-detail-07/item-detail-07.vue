		...
		<view v-if="!showList">
			<view class="detail-container">
				...
				<view class="boxline"  v-if="aboutList">
					<view class="box" >
						<view class="ssc" v-for="(ccode, index) in aboutList['current_code']" v-if="index<=9" :key="code+'-'+index" :class="Number(ccode)>40&&index<20? 'bigBall':(index==20?'lastBall':'')">{{ ccode }}</view>
						</br>
						<view class="ssc" v-for="(ccode, index) in aboutList['current_code']" v-if="index>9" :key="code+'-'+index" :class="Number(ccode)>40&&index<20? 'bigBall':(index==20?'lastBall':'')">{{ ccode }}</view>
					</view>
				</view>
		...
		numberRoll() {
			...
			that.rl = setInterval(() => {
				...
				that.aboutList.current_code.forEach((v, k) => {
					that.$set(that.aboutList.current_code,k,Math.floor(49 * Math.random()) + 1);
				});
			}, 200);
		},
		getAnalysisSort() {

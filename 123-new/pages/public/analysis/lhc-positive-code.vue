	...
	<view class="container">
		...
		<view class="echart">
			<view class="census" v-if="currentSort == 'temaMantissaHandDown' || currentSort == 'twoSidesAnalyze'">
				<view class="txt">特码两面分析(100)期</view>
			</view>
			<view class="census" v-else>
				<view class="txt">统计期数：</view>
		...
		<view class="t-popup" v-if="showInputBox">
			<view class="p-content">
				<view class="title">输入统计期数</view>
				<view class="font_icon iconsyclose"  @click="closePopBox"></view>
				<input type="text" class="num" placeholder="100" v-model="periods"/>
				<view class="error"><text v-show="errorShow">请输入正确的期数!</text></view>
				<button type="default" class="btn" :class="{on:checked==1}" @click="getPeriods" >确定</button>
			</view>
		</view>
	</view>
</template>

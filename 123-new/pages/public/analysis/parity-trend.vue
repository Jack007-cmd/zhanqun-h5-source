<template name="parity-trend">
	<view class="container" v-if="mainData">
		<view class="pattern">
			<view class="mode" :class="{ checked: option == 'A' }" @tap.stop="choseMode('A')">奇偶个数</view>
			<view class="mode" :class="{ checked: option == 'B' }" @tap.stop="choseMode('B')">奇偶比</view>
		</view>
		<view class="btn-list">
			...
			<table border="0" cellspacing="1" cellpadding="1">
				<thead>
					<tr>
						...
						<th colspan="4" style="width: 12%;" class="qiyu_arr">奇数个数</th>
						<th colspan="4" style="width: 12%;" class="qiyu_arr">偶数个数</th>
					</tr>
					<tr>
				...
				<tbody class="allNumber">
					<template v-for="(item, index) in mainData.list">
						<tr v-if="index < tPeriods">
							...
							<template v-for="(bn, bi) in item.list['o_num']">
								<td :class="{ hot: bn == 0, pol: bn == 0 }">
									<view>{{ bn == 0 ? bi : bn }}</view>
								</td>
							</template>
							<template v-for="(sn, si) in item.list['e_num']">
				...
				<tbody>
					<tr class="ptin">
						...
						<th colspan="4" class="qiyu_arr">奇数个数</th>
						<th colspan="4" class="qiyu_arr">偶数个数</th>
					</tr>
					<tr class="ptin">
						<th style="width: 4%;">1</th>
						<th style="width: 4%;">2</th>
					...
					<tr>
						...
						<template v-for="b in mainData.statistics.appear['o_num'][0]">
							<td>{{ b }}</td>
						</template>
						<template v-for="s in mainData.statistics.appear['e_num'][0]">
							<td class="yu">{{ s }}</td>
						</template>
					</tr>
					<tr>
						...
						<template v-for="b in mainData.statistics.avg['o_num'][0]">
							<td>{{ b }}</td>
						</template>
						<template v-for="s in mainData.statistics.avg['e_num'][0]">
							<td class="yu">{{ s }}</td>
						</template>
					</tr>
					<tr>
						...
						<template v-for="b in mainData.statistics.maxAway['o_num'][0]">
							<td>{{ b }}</td>
						</template>
						<template v-for="s in mainData.statistics.maxAway['e_num'][0]">
							<td class="yu">{{ s }}</td>
						</template>
					</tr>
					<tr>
						...
						<template v-for="b in mainData.statistics.max['o_num'][0]">
							<td>{{ b }}</td>
						</template>
						<template v-for="s in mainData.statistics.max['e_num'][0]">
							<td class="yu">{{ s }}</td>
						</template>
					</tr>
				</tbody>
			...
			<table border="0" cellspacing="1" cellpadding="1">
				<thead>
					<tr>
						...
						<th colspan="4" class="qiyu_arr">奇偶比</th>
					</tr>
					<tr>
						<th style="width: 5%;">奇</th>
						<th style="width: 5%;">偶</th>
						<th style="width: 5%;">奇</th>
						<th style="width: 5%;">偶</th>
						<th style="width: 5%;">奇</th>
						<th style="width: 5%;">偶</th>
				...
				<tbody class="allNumber">
					<template v-for="(item, index) in mainData.list">
						<tr v-if="index < tPeriods">
							...
							<template v-for="(temp, tempIndex) in item.list.sad">
								<td :class="{ hot: temp.s == 0, one: temp.s == 0 }">
									<view :style="{ background: temp.s == 0 ? 'transparent' : '' }">
									{{ temp.s == 0 ? '奇' : temp.s }}
									</view>
								</td>
								<td :class="{ hot: temp.d == 0, top: temp.d == 0 }">
									<view :style="{ background: temp.d == 0 ? 'transparent' : '' }">{{ temp.d == 0 ? '偶' : temp.d }}</view>
								</td>
							</template>
							<td
								:class="{ hot: item.list['oe_scale'][0]['3:0'] == 0 }"
								:style="{ background: item.list['oe_scale'][0]['3:0'] == 0 ? '#00BAFF' : '', 'font-zise': item.list['oe_scale'][0]['3:0'] ? '13px' : '' }"
							>
								<view :style="{ background: item.list['oe_scale'][0]['3:0'] == 0 ? 'transparent' : '' }">
									{{ item.list['oe_scale'][0]['3:0'] == 0 ? '3:0' : item.list['oe_scale'][0]['3:0'] }}
								</view>
							</td>
							<td
								:class="{ hot: item.list['oe_scale'][0]['2:1'] == 0 }"
								:style="{ background: item.list['oe_scale'][0]['2:1'] == 0 ? '#fcb103' : '', 'font-zise': item.list['oe_scale'][0]['2:1'] ? '13px' : '' }"
							>
								<view :style="{ background: item.list['oe_scale'][0]['2:1'] == 0 ? 'transparent' : '' }">
									{{ item.list['oe_scale'][0]['2:1'] == 0 ? '2:1' : item.list['oe_scale'][0]['2:1'] }}
								</view>
							</td>
							<td
								:class="{ hot: item.list['oe_scale'][0]['1:2'] == 0 }"
								:style="{ background: item.list['oe_scale'][0]['1:2'] == 0 ? '#00BAFF' : '', 'font-zise': item.list['oe_scale'][0]['1:2'] ? '13px' : '' }"
							>
								<view :style="{ background: item.list['oe_scale'][0]['1:2'] == 0 ? 'transparent' : '' }">
									{{ item.list['oe_scale'][0]['1:2'] == 0 ? '1:2' : item.list['oe_scale'][0]['1:2'] }}
								</view>
							</td>
							<td
								:class="{ hot: item.list['oe_scale'][0]['0:3'] == 0 }"
								:style="{ background: item.list['oe_scale'][0]['0:3'] == 0 ? '#fcb103' : '', 'font-zise': item.list['oe_scale'][0]['0:3'] ? '13px' : '' }"
							>
								<view :style="{ background: item.list['oe_scale'][0]['0:3'] == 0 ? 'transparent' : '' }">
									{{ item.list['oe_scale'][0]['0:3'] == 0 ? '0:3' : item.list['oe_scale'][0]['0:3'] }}
								</view>
							</td>
						</tr>
					</template>
				...
				<tbody>
					<tr>
						<td colspan="2">出现次数</td>
						<template v-for="sad in mainData.statistics.appear.sad">
							<td>{{ sad['s'] }}</td>
							<td>{{ sad['d'] }}</td>
						</template>
						<td>{{ mainData.statistics.appear['oe_scale'][0]['3:0'] }}</td>
						<td>{{ mainData.statistics.appear['oe_scale'][0]['2:1'] }}</td>
						<td>{{ mainData.statistics.appear['oe_scale'][0]['1:2'] }}</td>
						<td>{{ mainData.statistics.appear['oe_scale'][0]['0:3'] }}</td>
					</tr>
					<tr>
						<td colspan="2">平均遗漏</td>
						<template v-for="sad in mainData.statistics.avg.sad">
							<td>{{ sad['s'] }}</td>
							<td>{{ sad['d'] }}</td>
						</template>
						<td>{{ mainData.statistics.avg['oe_scale'][0]['3:0'] }}</td>
						<td>{{ mainData.statistics.avg['oe_scale'][0]['2:1'] }}</td>
						<td>{{ mainData.statistics.avg['oe_scale'][0]['1:2'] }}</td>
						<td>{{ mainData.statistics.avg['oe_scale'][0]['0:3'] }}</td>
					</tr>
					<tr>
						<td colspan="2">最大连出</td>
						<template v-for="sad in mainData.statistics.maxAway.sad">
							<td>{{ sad['s'] }}</td>
							<td>{{ sad['d'] }}</td>
						</template>
						<td>{{ mainData.statistics.maxAway['oe_scale'][0]['3:0'] }}</td>
						<td>{{ mainData.statistics.maxAway['oe_scale'][0]['2:1'] }}</td>
						<td>{{ mainData.statistics.maxAway['oe_scale'][0]['1:2'] }}</td>
						<td>{{ mainData.statistics.maxAway['oe_scale'][0]['0:3'] }}</td>
					</tr>
					<tr>
						<td colspan="2">最大遗漏</td>
						<template v-for="sad in mainData.statistics.max.sad">
							<td>{{ sad['s'] }}</td>
							<td>{{ sad['d'] }}</td>
						</template>
						<td>{{ mainData.statistics.max['oe_scale'][0]['3:0'] }}</td>
						<td>{{ mainData.statistics.max['oe_scale'][0]['2:1'] }}</td>
						<td>{{ mainData.statistics.max['oe_scale'][0]['1:2'] }}</td>
						<td>{{ mainData.statistics.max['oe_scale'][0]['0:3'] }}</td>
					</tr>
				</tbody>
			</table>
		</view>
...
export default {
	...
	methods: {
		...
		getAnalysisDetail() {
			let that = this;
			this.$api.getAnalysisDetail({ code: this.code, type: 'dszs', date: this.date }).then(res => {
				if (res.code == 0) {
					var arr = [];
					...
					that.mainData.list.forEach(v => {
						v.list['o_num'] = v.list['o_num'][0];
						v.list['e_num'] = v.list['e_num'][0];
					});
					setTimeout(function() {
						that.filterCtrl();				
					}, 100);

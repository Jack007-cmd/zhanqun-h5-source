			...
			<view :key="index">
				<view  class="content-list">
					<view class="content-title">
						<view class="title-left">累计：<text v-for="(k,cindex) in item.statistics">{{cindex+'（' + k + '）'}}</text></view>
							...
							<tbody>
								<tr class="tablebox">
									<td v-for="(temp, tempIndex) in item.statisticsList" :key="tempIndex">
										<p v-for="(a, aIndex) in temp" :class="a == '双'||a == '大'||a == '家'||a == '红'?'font_red':(a =='绿'?'font_green':(a=='蓝'?'font_blue':''))">{{a}}</p>
									</td>
								</tr>
							</tbody>
						</table>
...
<script>
	export default {
		name: 'lhc-luzhu',
		props: ['code', 'date', 'current_number','list'],
		data() {
			return {
				mainData: null,
